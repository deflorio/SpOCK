...
namespace gravity
    ...
    //d_Pnm_norm = ( (sqrt(1.0 - x*x))*Legendre_norm(n,m+1,x) + m*x*Legendre_norm(n,m,x) )/(x*x -1.0);
                                //d_Pnm_norm = ( -(n + 1.0)*x*Legendre_norm(n,m,x) + (n - m + 1.0)*Legendre_norm(n+1,m,x) )/(x*x -1.0);
    ...
    Vec3d GRAV::field_vec(double time,
                          const Ref<const VectorXd>& orbstate)
                            {
                            //P = MatnMAXxnMAXd::Zero();
                            //Pd = MatnMAXxnMAXd::Zero();
                            //Sigma1 = VectornMAXd::Zero();
                            //sin_mlambda = VectornMAXd::Zero();
                            //cos_mlambda = VectornMAXd::Zero();
                            ...
                            //Vec3d SC_posECEF = Vec3d::Zero();
                            //Vec3d acc_ECEF = Vec3d::Zero();
                            //Vec3d lonlath = Vec3d::Zero();
    ...
    //------------------------------------------------------------------------------
    // Method MatrixXd Legendre_norm_FR(int n, int m, double u)
    //------------------------------------------------------------------------------
    /**
     * Normalized associated Legendre functions
     *
     * @param u		cos(theta) where theta + phi  = pi/2.0 and phi = geocentric latitude
     * @param u		sin(theta)
     *
     * @return		Return the matrix whose elements are the value of the normalized associated Legendre function of degree n and order m
     * 			corresponding to u = cos(theta). The computation of the Legendre functions is based on a forward row (FR) recursive method
     */
    //------------------------------------------------------------------------------
    //MatrixXd GRAV::Legendre_norm_FR(double u,
    //                                double w)
    //                            {
    //                            double g, h, j;
    //                            //int n, m;
    ...
    //                            
    //                            MatrixXd L_FR;
    //                            L_FR.resize(rows,cols);
    //                            L_FR(0,0) = 1.0;
    //                            L_FR(1,1) = sqrt(3.0)*u;
    //                            // Compute L_FR(1,0) with the forward raw recursive relation
    //                            L_FR(1,0) = w*sqrt(3)*L_FR(0,0);
    //                            
    //                            //double prod_pi;
    //                            //Compute diagonal elements of L_FR
    //                            #pragma omp parallel for collapse(1)
    //                            for(int n = 2; n <= n_max; n++) L_FR(n,n) = u*sqrt( (2.0*n + 1.0)/(2.0*n) )*L_FR(n-1,n-1);
    //                            
    //                            //#pragma omp parallel for collapse(1)
    //                            for(int n = 2; n <= n_max; n++)
    //                                {
    ...
    //                                    else if(m > 0) j = 1.0;
    //                                    
    //                                    g = 2.0*(m + 1.0)/sqrt( (n - m)*(n + m + 1.0) );
    //                                    
    //                                    h = sqrt( (n + m + 2.0)*(n - m - 1.0)/( (n - m)*(n + m + 1.0) ) );
    //                                    
    //                                    if( (m+2) > n) L_FR(n,m) = g*(w/u)*L_FR(n,m+1)/sqrt(j);
    //                                    else L_FR(n,m) = ( g*(w/u)*L_FR(n,m+1) - h*L_FR(n,m+2) )/sqrt(j);			
    //                                    }
    //                                }
    //                            
    //                            return(L_FR);
    ...
    //------------------------------------------------------------------------------------------
    // Method MatrixXd d_Legendre_norm_FR(const Ref<const MatrixXd>& P, double u, double w);
    //---------------------------------------------------------------------------------------
    /**
     * First derivative of normalized associated Legendre functions
     *
     * @param L_FC   Matrix of normalized associated Legendre function
     * @param u	      cos(theta) where theta + phi  = pi/2.0 and phi = geocentric latitude
     * @param u		 sin(theta)
     *
     * @return		Return the matrix whose elements are the value of the first derivative of the normalized
     *              associated Legendre function of degree n and order m corresponding to u = cos(theta)
     *
     */
    //------------------------------------------------------------------------------
    //MatrixXd GRAV::d_Legendre_norm_FR(const Ref<const MatrixXd>& L_FR,
    //                                double u,
    //                                double w)
    //                                {
    //                                double e, j;
    ...
    //                                        else if(m > 0) j = 1.0;
    //                                        
    //                                        e = sqrt( (n + m + 1.0)*(n - m )/j );
    //                                        
    //                                        if(n == m) L_FR_d(n,m) = m*(w/u)*L_FR(n,m);
    //                                        else L_FR_d(n,m) = m*(w/u)*L_FR(n,m) - e*L_FR(n,m+1);
    //                                        }
    //                                    }
    //                                   
    //                                return(L_FR_d);
    //                                };
                            ...
                            //VectorXd Sigma1 = VectorXd::Zero(n_max+1);
                            //Sigma1 = VectornMAXd::Zero();
                            //VectorXd Sigma1(n_max+1);
                            //#pragma omp parallel for
                            //for(int i = 0; i <= n_max; i++) Sigma1(i) = 0.0;
                            // Partial derivative of potential U with respect to r
                            //#pragma omp parallel for collapse(1)
                            //for(int n = 2; n <= n_max; n++)
                            //    {
                            ...
                            //        double Cnm = C(n,m);
                            //        double Snm = S(n,m);
                            //        //cout << "P(n,m) = " << P(n,m) << "   Legendre_norm(n,m,sinphi) = " << Legendre_norm(n,m,sinphi) << endl;
                            //        Sigma1(n) = Sigma1(n) + (n + 1.0)*q*Pnm*( Cnm*cos(m*lambda) + Snm*sin(m*lambda) );
                            //        }
                            //    }
                            //
                            //dUdr = -( mu/(r*r) )*(1.0 + Sigma1.sum());
                            ...
                            //        double Snm = S(n,m);
                            //        
                            //        Sigma1(n) = Sigma1(n) + q*dPnmdphi*( Cnm*cos(m*lambda) + Snm*sin(m*lambda) );
                            //        }
                            //    }
                            //
                            //dUdphi = (mu/r)*Sigma1.sum();
                            ...
                            //        double Snm = S(n,m);
                            //        
                            //        Sigma1(n) = Sigma1(n) + m*q*Pnm*( -Cnm*sin(m*lambda) + Snm*cos(m*lambda) );
                            //        }
                            //    }
                            //
                            //dUdlambda = (mu/r)*Sigma1.sum();
                            ...
                            //MatrixXd L, Ld;
                            //L.resize(n_max+1,n_max+1);
                            //Ld.resize(n_max+1,n_max+1);
                            //
                            ////Compute all terms of L and Ld matrices
                            //for(int n = 2; n <= n_max; n++)
                            //    {
                            //    for(int m = 0; m <= n; m++)
                            //        {
                            //        L(n,m) = Legendre_norm(n,m,sinphi);
                            //        Ld(n,m) = d_Legendre_norm(n,m,sinphi);
                            //        }
                            //    }
                            //    
                            //for(int n = 0; n <= n_max; n++)
                            ...
                            //        {
                            //        cout.precision(20);
                            //        //cout << "P(" << n << "," << m << ") = " << P(n,m) << "   L(" << n << "," << m << ") = " << L(n,m) << endl;
                            //        if(P(n,m) - L(n,m) > 1E-12) cout << "P(" << n << "," << m << ") - " << "L(" << n << "," << m << ") = " << P(n,m) - L(n,m) << endl;
                            //        }
                            //    }
                            //    
                            //for(int n = 0; n <= n_max; n++)
                            ...
                            //        {
                            //        cout.precision(20);
                            //        //cout << "Pd(" << n << "," << m << ") = " << Pd(n,m) << "   Ld(" << n << "," << m << ") = " << Ld(n,m) << endl;
                            //        if(-Pd(n,m) - cosphi*Ld(n,m) > 1E-12) cout << "Pd(" << n << "," << m << ") - " << "Ld(" << n << "," << m << ") = " << -Pd(n,m) - cosphi*Ld(n,m) << endl;
                            //        }
                            //    }
                            //    
                            //exit(0);
